// StreamLayer Käyttäjäseuranta (Paikallinen)
const analytics = {
    getUserId: () => {
        // Päivitetty avain: streamlayer-user-id
        let userId = localStorage.getItem('streamlayer-user-id');
        if (!userId) {
            userId = 'sl_user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
            localStorage.setItem('streamlayer-user-id', userId);
            // Väri vaihdettu StreamLayer Blueksi (#007aff)
            console.log(`%c[StreamLayer] Uusi käyttäjä: ${userId}`, 'color: #007aff; font-weight: bold;');
        }
        return userId;
    },
    ...
    trackVisit: () => {
        ...
        const visitCount = parseInt(localStorage.getItem('streamlayer-visit-count') || '0') + 1;
        localStorage.setItem('streamlayer-visit-count', visitCount.toString());
        
        const uniqueUsers = parseInt(localStorage.getItem('streamlayer-unique-users') || '0');
        const isFirstVisit = visitCount === 1;
        
        if (isFirstVisit) {
            localStorage.setItem('streamlayer-unique-users', (uniqueUsers + 1).toString());
            console.log(`%c[StreamLayer] Uniikit käyttäjät: ${uniqueUsers + 1}`, 'color: #007aff; font-weight: bold;');
        }
        
        console.log(`%cKäyttäjä: ${userId} | Käynnit: ${visitCount} | Laite: ${analytics.getDeviceType()}`, 
                    'color: #888; font-size: 11px;');
        
        return { userId, visitCount, isFirstVisit, uniqueUsers: uniqueUsers + 1 };
    },
    
    getDeviceType: () => {
        const ua = navigator.userAgent;
        // Hyödynnetään olemassa olevaa mobiilirajaa (768px)
    ...
    getStats: () => {
        return {
            uniqueUsers: parseInt(localStorage.getItem('streamlayer-unique-users') || '0'),
            totalVisits: parseInt(localStorage.getItem('streamlayer-visit-count') || '0')
        };
    }
};

// Suoritetaan seuranta sivun latautuessa
